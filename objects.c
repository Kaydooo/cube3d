...
void	change_door_status(t_data *data, int count)
{
	...
	while (++y < data->map_height)
	{
		...
		while (++x < data->map_width)
		{
			...
			if (data->map[y][x] == DOOR_MAP_C && !status)
				obj_status(data, x, y, DOOR_CLOS);
			else if (data->map[y][x] == DOOR_MAP_C && status != DOOR_CLOS)
				obj_status(data, x, y, status - 1);
			else if (data->map[y][x] == DOOR_MAP_O && !status)
				obj_status(data, x, y, DOOR_OPEN);
			else if (data->map[y][x] == DOOR_MAP_O && status != DOOR_OPEN)
				obj_status(data, x, y, status + 1);
		}
	}
}
...
void	change_flame_status(t_data *data, int count)
{
	...
	while (++y < data->map_height)
	{
		...
		{
			status = obj_status(data, x, y, 2);
			if (data->map[y][x] == FLAME_MAP_F && !status)
				obj_status(data, x, y, FLAME_FRST);
			else if (data->map[y][x] == FLAME_MAP_F && status < FLAME_LAST)
				obj_status(data, x, y, status + 1);				
			else if (data->map[y][x] == FLAME_MAP_F && status == FLAME_LAST)
				obj_status(data, x, y, FLAME_FRST);
		}
